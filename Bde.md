...
**Fix:** `compactForecastVersions()` — strips the heavy per-version Summary/Super/rules copies (recoverable from their live synced keys) and drops the OLDEST versions until the whole set serializes under ~350 KB (keep ≥3; backstop caps 8 manual / 4 auto). Removed those heavy fields from `_pushVersion` + `ensureDailyBackupVersion` going forward. Runs after every save AND **on load (self-heal)** — so an already-oversized doc shrinks on the next refresh and writes fit again. `version.txt`→`-compact-versions-1mb`.
**Durable follow-up (deferred):** move `forecast_versions` to a per-version subcollection so the state doc never carries them — the definitive structural fix if versions grow again.

### 2026-07-06 — SERVER-SIDE stale-write gate (`_wv`) — the definitive fix
After proving that a stale tab (Don's, still open on old code) kept re-clobbering the restored close dates no matter what — because relying on users to refresh/close is unreliable — added a server-enforced gate. Every `fbPushNow` write to `dashboard/state` now stamps **`_wv: WRITE_VERSION`** (=20260706). Carlos publishes a Firestore rule that rejects any write with `_wv` below the threshold → an out-of-date tab (old code has no `_wv`, and old fbPushNow used merge:false so the doc loses `_wv` → null) is **physically blocked from writing at the database**, regardless of user action. Reads stay open so stale tabs still display data. **Deploy order matters:** ship+load the `_wv` code FIRST (everyone who needs to write hard-refreshes), THEN publish the rule — otherwise all writes (incl. ours) are rejected. Bump `WRITE_VERSION` + rule threshold together to force-lock-out older clients on future critical changes. `version.txt`→`-write-version-gate`. Firestore rule text handed to Carlos.
...
### 2026-07-06 — ROOT CAUSE of the recurring 2030 close-date revert: whole-doc overwrite on every push
Don: "the system is overwriting my changes, most notably close date — dates that were final 2 weeks ago are all back to 2030, I've re-fixed 3-4×. Stopping the forecast until fixed." **TRUE ROOT CAUSE found:** `fbPushNow` wrote the ENTIRE state (all `FB_SYNCED_KEYS`) with **`merge:false`** on EVERY save — including the login housekeeping (daily backup / weekly trend snapshot). So whenever ANY user opened Horizon (or saved anything), that client re-wrote the whole `deals` array from ITS local copy; if that copy was even slightly stale (loaded just before Don's edit synced, or a long-lived/idle tab), it clobbered everyone's `deals`, reverting Don's manual `forecastClosingDate` → blank → falls back to Zoho's 2030 placeholder. This is why it recurred no matter how many times Don re-fixed it, and why it looked like "the system." The 2026-07-01 sync fix preserved overrides *within the sync path* but did NOT stop the *whole-doc push* clobber.
**Fix:** per-key dirty tracking + `merge:true`. `save()`→`fbSchedulePush(k)` records `_dirtyKeys[k]`; `fbPushNow` pushes ONLY the dirty keys and MERGES (so keys this client didn't touch — like someone else's `deals` edit — stay intact in Firestore); pushes nothing when clean. Login housekeeping now pushes only `trend_snapshots`/backup, NEVER `deals`. New `fbPushAll()` used once to create the doc. onSnapshot applies under `fbSuppressPush` so remote applies don't dirty local. This structurally ends the clobber; the only residual is two people editing `deals` in the same sub-second (conflict guard warns).
**Don's recovery:** his close-date edits are all in the audit log → Admin has "↩ Restore forecast close dates from audit" (re-applies the latest hand-entered date per deal) + the 🔎 Audit log to see the timeline. JS validated; `version.txt`→`-per-key-push-rootfix`.

### 2026-07-06 — No white-hover on ANY total row (swept the whole app)
Carlos (again): the Active Customers "TOTAL" bar went white/transparent on hover. Applied the `pl-total` class to the 3 remaining unprotected colored total rows: Active Customers grand total, the "Total HC" row + the "TOTAL" row in the trend/monthly-HC cards. Swept every dark/`color:white` `<tr>` in the app — the Aureum grid (`sumrow frz`), 3-mo & 12-month (`totals-row`), Super-Deals SUB-TOTAL (`totals-row`), Pipeline & the new phase table (`pl-total`) were already protected. CSS `tr.pl-total:hover td{background:transparent}` (0,2,2) beats `tr:hover td{background:#fafbfc}` (0,1,2) in every table incl. grid-tbl. See [[feedback_no_hover_flash]]. `version.txt`→`-total-rows-no-hover`.

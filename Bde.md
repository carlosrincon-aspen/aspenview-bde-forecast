...
>
> **⭐ Start here:** read the top Changelog entry (**2026-07-06 — Data-integrity crisis**) for the current data-persistence model, then **§9 Open items** for what's pending (publish the `_wv` Firestore rule; team hard-refresh; move versions to a subcollection). Live URL, repo & deploy in §3. Line numbers in §10 are stale — `grep` function names.
...
### Data integrity — do next (from the 2026-07-06 crisis; see the top changelog entry)
- ⏳ **Publish the `_wv` Firestore rule** (rule text handed to Carlos): rejects writes with `_wv` below `20260706`, so out-of-date tabs are blocked at the database. **Order:** everyone who writes must hard-refresh to the `_wv` code FIRST, then publish (else all writes get rejected). Reversible (revert to the open write rule).
- **Team hard-refresh once** — the sync/blocking fixes only protect tabs already on the current version; any tab left open on old code can still misbehave until refreshed.
- **Durable follow-up:** move `forecast_versions` to a per-version subcollection so the `dashboard/state` doc never carries them → the 1 MB limit can't recur. Meanwhile the Health Check "Firestore doc size" line is the early warning + "🗜 Compact versions now" is the manual lever.

### Blocking / setup
- ✅ **Zoho `Deals.WRITE` scope is LIVE** (since 2026-06-29) — Push works. Token is per-user/per-browser and expires ~1h → reconnect via the Zoho pill when it disconnects. (Editing never depends on Zoho — it saves to Horizon regardless.)
...
`index.html` is the entire app (~7,250 lines now). ⚠ **The line ranges below are approximate and drift as the file grows — don't trust the numbers; `grep` for the function name instead** (e.g. `grep -n "function renderOverview" index.html`).
...
### 2026-07-06 — Data-integrity crisis: found & fixed the recurring data loss ⭐ READ THIS for the current persistence model
Don's manual forecast close dates kept reverting to Zoho's 2030 placeholder (he re-fixed 3–4× over 2 weeks, then stopped forecasting). Investigation found **three compounding causes**, all fixed. This is the current sync/persistence model — read before touching Firestore/sync code.

**1) 1 MB document limit — the biggest cause.** The whole app state is ONE Firestore doc (`dashboard/state`) capped at 1,048,576 bytes; once over, EVERY write fails silently → edits saved locally but never reached the cloud → vanished on refresh. Bloat driver = `forecast_versions` (each version snapshots ALL ~317 deals; there were ~22). Fix: `compactForecastVersions()` strips heavy per-version copies (PMFF/Super/rules — recoverable from their live keys) and prunes oldest until the set serializes <~350 KB (keep ≥3; caps 8 manual / 4 auto); runs after every save AND on load (self-heal). Doc went ~1.06 MB → ~619 KB. `_pushVersion`/`ensureDailyBackupVersion` no longer store the heavy fields. Version labels now use **max-vN + 1** (monotonic; survives pruning — count-based broke and recycled "v6").

**2) Whole-doc clobber (root fix) — per-key merge push.** Before, `fbPushNow` wrote the ENTIRE state with `merge:false` on every save (incl. login housekeeping), so any slightly-stale client overwrote everyone's `deals` → wiped Don's dates. Now `save(k,v)`→`fbSchedulePush(k)` records `_dirtyKeys[k]`; `fbPushNow` writes ONLY the dirty keys with `merge:true`, and pushes nothing when clean. Login housekeeping (daily backup / trend snapshot) can no longer touch `deals`. `fbPushAll()` creates the doc the first time. onSnapshot applies remote under `fbSuppressPush` so it never dirties local. Residual: two people editing `deals` in the same sub-second → conflict guard warns.

**3) Stale tabs.** (a) On-refresh version-check now **hard-blocks Sync & Push** on an out-of-date tab — `_updateAvailable` + `blockStale()` guard at the top of `syncZohoDeals` / `lockAndPushForecast` / `pushApprovedToZoho` (poll 60s + on focus; flushes pending edits first so nothing is lost; fail-open if version.txt unreadable). (b) Server-side gate: every write stamps `_wv: WRITE_VERSION` (=20260706); a Firestore rule rejecting writes below the threshold physically blocks old clients regardless of user action. ⏳ **Rule text handed to Carlos but NOT yet published** — see Open items (§9). Bump `WRITE_VERSION` + rule together to force-lock-out old clients on future critical changes.

**Also shipped today:** conflict-guard false-positive fixed (login housekeeping no longer arms it — `NON_EDIT_KEYS`); conflict banner (teammate saved over your unsaved edits → Reload / Keep mine); conflict guard never loses in-progress edits (flush before blocking); audit load/recover raised 1000/1500 → **8000** events; Zoho **>2000 pagination** fixed (cursor `page_token` in `fetchAllZohoDeals`/`fetchAllZohoAccounts`); **no white-hover on ANY total row** (swept app — `pl-total`/`sumrow`/`totals-row`; see [[feedback_no_hover_flash]]); Health Check now watches **doc size** (green/amber/red) + **"🗜 Compact versions now"** button in Admin → Recovery & data.

**Recovery used & verified:** Admin → Activity → **"↩ Restore forecast close dates"** re-applies the latest hand-entered date per deal from the permanent `activity_archive`. Don's dates restored (55/317, incl. Warner Bros 2026-12-31), confirmed persisted after a hard-refresh, saved as **v20**. Don back to forecasting.

**Ops lesson:** GitHub Pages `deploy` job hit transient "Deployment failed, try again later" + a stuck queued run after ~13 rapid deploys. **Batch changes; don't push in rapid succession.** A clean empty-commit redeploy unsticks it; verify each deploy landed via live `version.txt` before the next.
